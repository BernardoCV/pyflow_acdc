...
### 2.5 Heat-pump flexible load class (CIRED 2026 resilience / FEL extension)

References:

- `citcea_extras_pyflow/heat_pumps_OPF/CIRED2026_Enhancing_Distribution_System_Resilience_through_Energy_Communities.pdf`, §3 and §4
- `citcea_extras_pyflow/heat_pumps_OPF/ILEC_full_code.py` (Montse prototype)

This is **not part of the shipped BESS / H2 path**. It is a **future extension** that should reuse the same OPF integration style already used for `Storage_*` and `Electrolyser`: explicit classes, nodal hooks, snapshot OPF support, and time-coupled state updates in sequential / window modes.
...
**Observed Montse pattern in `ILEC_full_code.py`**
...
**Planning-oriented interpretation** (paper Eq. 4-6 + Montse prototype)
...
**Operation-oriented FEL interpretation** (paper Eq. 11-19)
...
**Implementation choice for pyflow_acdc**
...
**Implementation notes for pyflow_acdc**
...
| `Classes.py` (future HP extension) | `HeatPump`, `Grid.heat_pumps`, `Node_AC.connected_heat_pumps` |
| `grid_modifications.py` (future HP extension) | `add_heat_pump()` with baseline / flexibility envelope inputs |
| `ACDC_OPF_NL_model.py` (future HP extension) | HP active/reactive load vars, flexibility-state constraints, AC nodal load hook |
| `window_opf.py`, `Time_series.py` (future HP extension) | sequential / coupled `E_flex` updates and terminal neutrality rules |
| `Results_class.py` (future HP extension) | HP schedule, comfort-state, and reserve-envelope reporting |
...
### Secondary reference (heat-pump flexibility / resilience)

M. Montalà-Palau, J. J. Markus, M. Kazemi, M. Cheah Mañé, C. Papadimitriou, and O. Gomis-Bellmunt, *Enhancing Distribution System Resilience through Energy Communities*, CIRED 2026 Brussels Workshop, Paper 1361, 2026.
