...
def TS_ACDC_PF(grid, start=1, end=99999, OPF=False, VarPrice=False):
    index = start-1
    ...
    while index < max_time:
        price_data = {'time': index+1}
        for ts in grid.Time_series:
            typ = ts.type
            if typ == 'Load':
                ts.node.PLi = ts.TS[index]
            elif typ == 'Slack':
                if VarPrice == True:
                    ts.node.price = ts.TS[index]
                    price_data[f'{ts.name}_price'] = ts.TS[index]
                    grid.VarPrice = True

            else:
                ts.node.PGi = ts.TS[index]

            # ts.node.P_AC = ts.node.PGi-ts.node.PLi
            # ts.node.Q_AC = ts.node.QGi-ts.node.QLi

        if OPF == True:
            [model,results]=OPF_convAC(grid)
            [opt_res_P_conv_DC, opt_res_P_conv_DC_dict,opt_res_P_extGrid] = OPF_dict_results_PDC(grid,model)

            opt_res_P_conv_DC_dict['time'] = index+1
            opt_res_P_extGrid['time'] = index+1
        for conv in grid.Converters_ACDC:
            ...
            if conv.type == 'Droop' or conv.type== 'P':
                if OPF == True:
                    ...
                    conv.P_DC = grid.Pconv_save[conv.ConvNumber]
                    s = 1
        ...
        money_data = {'time': index+1}
        row_data = {'time': index+1}
        in_data = {'time': index+1}
        line_data = {'time': index+1}
        grid_data = {'time': index+1}
        ...
        if OPF == True:
            grid.Time_series_Opt_res_P_conv_DC.append(
                opt_res_P_conv_DC_dict)
            grid.Time_series_Opt_res_P_extGrid.append(opt_res_P_extGrid)
        # print(index+1)
        index += 1
    # Create the DataFrame from the list of rows
    if OPF == True:
        grid.Time_series_Opt_res_P_conv_DC = pd.DataFrame(
            grid.Time_series_Opt_res_P_conv_DC)
        grid.Time_series_Opt_res_P_extGrid = pd.DataFrame(
            grid.Time_series_Opt_res_P_extGrid)
