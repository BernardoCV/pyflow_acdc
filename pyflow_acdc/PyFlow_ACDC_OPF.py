...
def OPF_ACDC(grid,ObjRule=weights_def,kap=1e-3):
    model= OPF_createModel(grid)
...
def OPF_createModel(grid,V_inf=0.9,V_up=1.1):
    ...
    ### AC Variables
    #AC nodes variables
    model.V_AC       = pyo.Var(model.nodes_AC, bounds=(V_inf, V_up), initialize=V_ini_AC)
    ...
    ### DC variables
    #DC nodes variables
    model.V_DC = pyo.Var(model.nodes_DC, bounds=(V_inf, V_up), initialize=V_ini_DC)
    ...
    ### Converter Variables
    model.Uc   = pyo.Var(model.conv, bounds=(V_inf, V_up), initialize=1) 
    model.Uf   = pyo.Var(model.conv, bounds=(V_inf, V_up), initialize=1) 
